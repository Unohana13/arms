...
<body>
	...
	<div class="container" colspan="8" align="center">
		...
		if(isset($_REQUEST['tb1'])) {
			...
			$sql_forms = $mysqli->query("SELECT * FROM tenant WHERE status !='Archived' AND  Tenant_Name='.$condition.'");
			$pages->items_total = $sql_forms->num_rows;
			$pages->mid_range = 9;
			$pages->paginate();

			$result	=	$mysqli->query("SELECT * FROM tenant WHERE status !='Archived' AND Tenant_Name='.$condition.' ORDER BY Tenant_Name ASC ".$pages->limit."");
		}else {
      $pages = new Paginator;
			$pages->default_ipp = 10;
			...
			$result	=	$mysqli->query("SELECT * FROM tenant where status !='Archived' ORDER BY Tenant_Name ASC ".$pages->limit."");
    }
		?>
		...
		<table class="table table-bordered table-striped" id="empTable">
          ...
          <td style="text-align:center;">
              ...
              <i class="fas fa-eye" data-toggle="tooltip" title="More Information"></i></a>
				  ...
				  <a class ="editstatus" href="#status_edit<?php echo $val['Tenant_Id'];?>" data-toggle="modal"><i class="fas fa-trash" data-toggle="tooltip" title="View ID"></i></a>
					...
					<div id="status_edit<?php echo $val['Tenant_Id'];?>" class="modal fade">
							<div class="modal-dialog">
								<div class="modal-content">
									<form method="post" action="addtenant.php">
										<div class="modal-header">
											<h4 class="modal-title"><?php echo $val['Tenant_Name']; ?>'s Tenant Status</h4>
									</div>
											<div class="modal-body">
											<input name="editTenantid" type = "hidden" class="form-control" value="<?php echo $val['Tenant_Id']; ?>">
											<input name="editstatusroom" type = "hidden" class="form-control" value="<?php echo $val['roomnumber']; ?>">
											<input name="editstatuslocation" type = "hidden" class="form-control" value="<?php echo $val['location']; ?>">
											...
											<div class="modal-footer">
															...
															<input type="submit" onsubmit="setTimeout(function () { window.location.reload(); }, 10)" class="btn btn-success" name="edittenantstatus" value="Apply">
											</div>

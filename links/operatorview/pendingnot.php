...
<body>
	...
	<div class="container" colspan="8" align="center">
		...
		if(isset($_REQUEST['tb1'])) {
			...
			{
				$condition		.=	" AND Tenant_Name LIKE'%".$_GET['tb1']."%' `Payment_Status` = 3 OR `Payment_Status` = 0";
			}
      ...
      $pages = new Paginator;
			...
			$result	=	$mysqli->query("SELECT * FROM tenant where 1 AND`Payment_Status` = 3 OR `Payment_Status` = 0 ORDER BY Tenant_Name ASC ".$pages->limit."");
    }
		?>
											...
											<form method="post">
												...
												<div class="modal-footer">
													...
													<input type="hidden" id="btnClickedValue" name="btnClickedValue" value="<?php echo $val['Tenant_Id']; ?>" />
														<input type="hidden" id="TenantRoomnumber" name="TenantRoomnumber" value="<?php echo $val['Room_Number']; ?>" />
					...
					<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
						...
						<div class="modal-body">
							<div class="form-group">
								<label>Room Number</label>
								<?php $initial = "0"; ?>
								<select id="select1" name="roomnumber" class="form-control">

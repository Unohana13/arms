...
<body>
	...
	<div class="container" colspan="8" align="center">
		<Strong><span>Room information <p class="fas fa-restroom"></p> &nbsp;</span></strong>
...
<form method="get" action="<?php echo $_SERVER['PHP_SELF'];?>">
	<div class="form-inline">
		<Strong><span>Search <p class="fas fa-search"></p> &nbsp;</span></strong>
			 <input type="text" name="tb1" onchange="submit()" class="form-control col-lg-7" placeholder="Enter Room Number">
			 <div class="col-lg-2">
		...
		if(isset($_REQUEST['tb1'])) {
			...
			{
				$condition		.=	" AND Room_Number LIKE'%".$_GET['tb1']."%'";
			}
			...
			$result	=	$mysqli->query("SELECT * FROM room WHERE 1 ".$condition." ORDER BY Room_Number ASC ".$pages->limit."");
		}else {
      $pages = new Paginator;
			$pages->default_ipp = 10;
			...
			$result	=	$mysqli->query("SELECT * FROM room ORDER BY Room_Number ASC ".$pages->limit."");
    }
		?>
          ...
          <th>Number Of Occupants</th>
			...
			<tbody>
				...
				<tr>
					...
					<td><?php echo $val['Number_Of_Beds']; ?></td>
          <td><?php echo $val['Number_Of_Occupants']; ?></td>
          <td><?php echo $val['Number_Of_Available_Beds']; ?></td>
				</tr>
				<?php
					}
				}else{?>
